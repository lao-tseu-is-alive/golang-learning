...
const (
  ...
  `
	// Database
	host             = "localhost"
	connectionString = "postgres://gouser:gouser-2019@%s/golangdb?sslmode=disable"
...
func main() {
	...
	if err := e.StartServer(server); err != nil {
		e.Logger.Errorf("Error starting the server %v\n", err)
	}
	e.Logger.Printf("Server listening on : %s PID:[%d]", server.Addr, os.Getpid())
}
