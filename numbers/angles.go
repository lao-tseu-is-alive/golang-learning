...
func main() {
	...
	valPi := radiansToDegrees(math.Pi)
	fmt.Printf("Pi radians (3.14159...) is : %.4f degrees\n", valPi)

	valDeg := degreesToRadians(val)
	fmt.Printf("%.4f degrees is %.4f rad\n", val, valDeg)

	valPiDeg := degreesToRadians(valPi)
	fmt.Printf("%.4f degrees is %.4f rad\n", valPi, valPiDeg)

	// Conversion as part of type methods
